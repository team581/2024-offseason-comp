...
class ProjectileMotion:
  ...
  def getpoints(self, vector: Vector, exit: Point):
    ...
    while note_position[-1].y >= 0:
      ...
      drag_force = -1 * self.air_resistance*(math.pow(note_velocity[-1].x,2) + math.pow(note_velocity[-1].y,2))
      acceleration = Point(drag_force*math.cos(angle),
                           (-9.8)+(drag_force*math.sin(angle)))
      velocity = Point(acceleration.x * self.dt + note_velocity[-1].x,
                       acceleration.y * self.dt + note_velocity[-1].y)
      position = Point(note_position[-1].x + velocity.x * self.dt,
                       note_position[-1].y + velocity.y * self.dt)
